...
What this script does:
  ...
  4. Extracts all 12 leads (I, II, III, aVR, aVL, aVF, V1-V6)
  5. Resamples all signals to SIGNAL_LENGTH = 1000 samples per lead
  6. Splits 80/20 into train and validation sets
  7. Saves:
       dataset/ptbxl_train_signals.npy  (N_train, 12, 1000)
       dataset/ptbxl_train_labels.npy   (N_train,)
       dataset/ptbxl_val_signals.npy    (N_val, 12, 1000)
...
# Lead names in wfdb PTB-XL order
...
def load_signal(ecg_id: int, db: pd.DataFrame) -> np.ndarray | None:
    """
    Load all 12 leads from a PTB-XL record.
    Uses _find_record() to locate the file in either valid disk layout.
    Returns a (12, SIGNAL_LENGTH) float32 array, or None on error.
    """
    ...
    try:
        ...
        # record.p_signal shape: (N_samples, 12)
        leads = []
        for lead_idx in range(12):
            lead = record.p_signal[:, lead_idx].astype(np.float32)
            if np.any(~np.isfinite(lead)):
                lead = np.nan_to_num(lead, nan=0.0, posinf=0.0, neginf=0.0)
            leads.append(resample(lead, SIGNAL_LENGTH).astype(np.float32))
        return np.stack(leads, axis=0)  # (12, SIGNAL_LENGTH)
    except Exception:
        return None
...
def prepare_dataset(max_records: int = 5000, val_fraction: float = 0.2, seed: int = 42):
    ...
    # ── Step 5: Load signals ───────────────────────────────────────────────
    print(f"\n[Step 5] Loading {len(valid_ids)} ECG signals (all 12 leads)...")
    signals = []
    labels = []
    failed = 0
    ...
    signals = np.array(signals, dtype=np.float32)   # (N, 12, SIGNAL_LENGTH)
