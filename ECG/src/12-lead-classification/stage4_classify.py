...
class ECGResNet1D(nn.Module):
    ...
    Each stage has 2 residual blocks (matches run_20260603_141028 checkpoint).
    """
    def __init__(self, num_classes: int = NUM_CLASSES, in_channels: int = 12):
        super().__init__()
...
class ECGDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.augment:
            ...
            t = np.linspace(0, 1, sig.shape[1])
            sig += 0.02 * np.sin(2 * np.pi * np.random.uniform(0.5, 2.0) * t)
            shift = np.random.randint(-20, 20)
            sig = np.roll(sig, shift, axis=1)
        # Per-lead normalisation: each of the 12 channels independently
        sig = sig - sig.mean(axis=1, keepdims=True)
        sig = sig / (sig.std(axis=1, keepdims=True) + 1e-8)
        return torch.from_numpy(sig), torch.tensor(self.labels[idx])  # (12, 1000)
...
def train_model(signals_train, labels_train, signals_val, labels_val,
    ...
    model = ECGResNet1D(in_channels=12).to(device)
...
def load_model(checkpoint_path: str) -> ECGResNet1D:
    model = ECGResNet1D(in_channels=12)
...
def classify(lead_signals: dict, model: ECGResNet1D, device: str = "cpu") -> dict:
    """
    lead_signals: dict mapping lead name -> np.ndarray (raw signal_mv from digitizer).
    Missing leads are zero-padded.
    Returns predicted class, confidence, and full probability breakdown.
    """
    stack = []
    for name in LEAD_ORDER:
        sig = lead_signals.get(name, np.zeros(SIGNAL_LENGTH, dtype=np.float32))
        sig = resample(sig, SIGNAL_LENGTH).astype(np.float32)
        sig = (sig - sig.mean()) / (sig.std() + 1e-8)
        stack.append(sig)
    arr = np.stack(stack, axis=0)                               # (12, 1000)
    tensor = torch.from_numpy(arr).unsqueeze(0).to(device)     # (1, 12, 1000)
...
if __name__ == "__main__":
    model = ECGResNet1D(in_channels=12)
    dummy = torch.randn(4, 12, SIGNAL_LENGTH)
    out = model(dummy)
