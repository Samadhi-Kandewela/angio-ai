...
Output: predicted masks with red bounding boxes around stenosis regions.
"""
import json
import os
import sys
...
OUTPUT_DIR = os.path.join(PROJECT_ROOT, "dataset", "stenosis", "val_predicted_masks")
IMAGE_SIZE = (512, 512)
...
for ann in coco_data["annotations"]:
    if ann["category_id"] == 26:
        ...
        stenosis_bboxes.setdefault(fname, []).append(ann["bbox"])
    ...
    for fname in image_files:
        ...
        if fname in stenosis_bboxes:
            for bbox in stenosis_bboxes[fname]:
                ...
                cv2.rectangle(mask_color, (x, y), (x + w, y + h), (0, 0, 255), 2)

        out_name = os.path.splitext(fname)[0] + ".png"
        cv2.imwrite(os.path.join(OUTPUT_DIR, out_name), mask_color)
        count += 1
