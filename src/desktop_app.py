"""
Real-Time Angiogram Segmentation Viewer
========================================
PyQt5 desktop application for real-time medical image segmentation.
Streams video via OpenCV, runs ONNX/PyTorch inference per frame,
and displays Original + Segmentation Overlay in dual panels.
...
# Both ship C++ runtime DLLs that conflict if loaded in wrong order.
# onnxruntime must load its DLLs first, otherwise PyQt5 claims them.
try:
...
from PyQt5.QtWidgets import (
    ...
    QGroupBox, QStatusBar, QSizePolicy, QFrame, QLineEdit
)
...
# NOTE: PyTorch is NOT imported at startup to avoid DLL conflicts on Windows.
# It is imported lazily inside load_model() only when a .pth file is selected.
# For best performance, use .onnx models (recommended).
...
class VideoThread(QThread):
    """
    Background thread that reads video frames, runs inference,
    and emits both the original and overlay frames to the GUI.
    """
    # Signals: original_frame, overlay_frame, fps, latency_ms
    frame_ready = pyqtSignal(np.ndarray, np.ndarray, float, float)
    finished = pyqtSignal()
    ...
    def load_model(self, model_path):
        ...
        if self._is_onnx:
            ...
            try:
                # Lazy import torch (avoids DLL conflicts on Windows)
                import torch
    ...
    def run(self):
        """Main loop: read frames → infer → emit."""
        ...
        while self._running and cap.isOpened():
            ...
            # Keep a Copy of the Original (Non-CLAHE) for Display
            img_resized = cv2.resize(frame, (512, 512))
            img_rgb_original = cv2.cvtColor(img_resized, cv2.COLOR_BGR2RGB)

            # Apply CLAHE (Contrast Limited Adaptive Histogram Equalization)
            # This matches the training data preprocessing in dataset.py
            ...
            # ... (inference code uses img_float) ...
            img_chw = np.transpose(img_float, (2, 0, 1))
            img_batch = np.expand_dims(img_chw, axis=0)
            
            # Inference Block Copied Here for Context (No Changes needed inside, just ensuring variable names match)
            # ...

            # Emit both frames: Send ORIGINAL for display, but use ENHANCED for overlay blending? 
            # Actually, overlay should probably be blended on top of the ENHANCED or ORIGINAL?
            # User wants to see "Original frame as actually original video look like".
            # So the first argument to emit should be `img_rgb_original`.
            # The second argument (blended) might look weird if we blend mask on original but model saw enhanced.
            # But usually blending on original is preferred for visualization.
            ...
            if self._is_onnx and self._ort_session:
                ...
                with _torch.no_grad():
                    ...
                    # Support CUDA if model is on GPU (though user likely on CPU)
                    if hasattr(self, '_torch_device') and self._torch_device:
                         tensor_in = tensor_in.to(self._torch_device)
            ...
            # Use img_rgb_original for display so user sees natural video
            ...
            # Emit: Original (Raw), Blended (Raw+Mask)
            self.frame_ready.emit(img_rgb_original, blended, fps, latency_ms)

            # Small sleep to not max out CPU when video is short
            self.msleep(1)
...
def ndarray_to_qpixmap(img_rgb: np.ndarray, target_w: int, target_h: int) -> QPixmap:
    ...
    return QPixmap.fromImage(qimg).scaled(target_w, target_h, Qt.KeepAspectRatio, Qt.SmoothTransformation)
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Angiogram Segmentation Viewer")
    ...
    def _build_ui(self):
        ...
        # ── Title ────────────────────────────────────────────────────
        title = QLabel("Real-Time Angiogram Segmentation System")
        title.setFont(QFont("Segoe UI", 24,QFont.Bold))
        title.setStyleSheet("color: #FFFFFF; padding-bottom: 6px;")
        ...
        self.label_original.setMinimumSize(480, 360)
        self.label_original.setStyleSheet("background-color: #161B22; border-radius: 8px; font-size: 14px; color: #484F58;")
        panel1_layout.addWidget(self.label_original)
        panels_layout.addWidget(panel1_box)

        # Panel 2: Overlay
        panel2_box = QGroupBox("Segmentation Overlay")
        panel2_layout = QVBoxLayout(panel2_box)
        self.label_overlay = QLabel("No video loaded")
        self.label_overlay.setAlignment(Qt.AlignCenter)
        self.label_overlay.setMinimumSize(480, 360)
        self.label_overlay.setStyleSheet("background-color: #161B22; border-radius: 8px; font-size: 14px; color: #484F58;")
        panel2_layout.addWidget(self.label_overlay)
        ...
        self.txt_video.setMinimumWidth(180)
        ...
        self.txt_model.setMinimumWidth(180)
        ...
        # ── Status Bar ───────────────────────────────────────────────
        self.statusBar().showMessage("Ready. Load a video and model to begin.")

    # ── Slot: frame received ─────────────────────────────────────────
    @pyqtSlot(np.ndarray, np.ndarray, float, float)
    def _on_frame(self, original, overlay, fps, latency_ms):
        w = self.label_original.width()
        h = self.label_original.height()

        self.label_original.setPixmap(ndarray_to_qpixmap(original, w, h))
        self.label_overlay.setPixmap(ndarray_to_qpixmap(overlay, w, h))
    ...
    def _stop(self):
        ...
        self.label_overlay.setText("No video loaded")
