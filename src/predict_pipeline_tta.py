"""
Same two-stage pipeline as predict_pipeline.py (MobileUNetv3 segmentation ->
MaskLocalizationNet anatomy localization), except the segmentation stage
uses flip test-time augmentation (see tta.py): +0.4-0.6pt Dice/IoU measured
on val/test with no precision/recall trade-off, at 4x the segmentation
forward-pass cost. Kept as a separate file so predict_pipeline.py stays
untouched.

The localization stage is intentionally NOT flip-augmented: flipping the
image would swap left/right coronary anatomy, so averaging flipped
localization predictions would corrupt the anatomy labels rather than
denoise them. Only the segmentation mask (which is left/right symmetric as
a "vessel or not" map) benefits from flip averaging.

Usage:
    python src/predict_pipeline_tta.py --image path/to/frame.png
    python src/predict_pipeline_tta.py --image path/to/frame.png --point 240,180
"""
...
def get_args():
    p = argparse.ArgumentParser(description="Run the full segmentation(+TTA)+localization pipeline on one image")
...
if __name__ == "__main__":
    ...
    print(f"Segmentation checkpoint: {args.seg_checkpoint} (flip-TTA enabled)")
    ...
    with torch.no_grad():
        seg_probs = predict_tta(seg_model, tensor)
        vessel_pred = (seg_probs > args.threshold).float()
    ...
    mask_path = os.path.join(output_dir, f"{stem}_vessel_mask_tta.png")
    vessel_overlay_path = os.path.join(output_dir, f"{stem}_vessel_overlay_tta.png")
    anatomy_overlay_path = os.path.join(output_dir, f"{stem}_anatomy_overlay_tta.png")
